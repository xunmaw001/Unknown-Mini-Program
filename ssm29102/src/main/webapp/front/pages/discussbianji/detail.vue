...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussbianjiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussbianjiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussbianji');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussbianji');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussbianji', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussbianji', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussbianji/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussbianji', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussbianji', this.detail);
				this.$utils.msg('审核成功');
